...
class Validator(NodeVisitor):
    ...
    VALIDATION_FINDING_UNHANDLED_EXCEPTION = ValidationFinding(
        ValidationFindingSeverity.FATAL,
        'base.unhandled_exception'
    )
    ...
    def validate_wrapper(self, node: PDUNode) -> 'ValidationResult':
        try:
            ...
            logger.exception('Unhandled exception occurred when executing '
                             'validator %s on node %s', self.name, node.path
                             )
            finding = ValidationFindingDescription(
                self.VALIDATION_FINDING_UNHANDLED_EXCEPTION,
                str(e)
            )
...
class ValidatorContainer(Validator):
    ...
    def _validate_rec(self, node: PDUNode,
    ...
    def validate(self, node: PDUNode):
        if node.pdu != self.value:
            ...
            raise ValidationFindingEncountered(
                validation,
                f'Expected="{self.value}", actual="{node.pdu}"'
            )
...
class ASN1ConstraintValidator(Validator):
    ...
    def _get_message(self, exc):
        if isinstance(self.constraint, PermittedAlphabetConstraint):
            return ('Invalid character outside permitted alphabet of "'
                    f'{"".join(self.constraint._values)}"'
                    )
        elif isinstance(self.constraint, ValueRangeConstraint):
            return ('Invalid value outside range '
...
class TypeMatchingValidator(Validator):
    def __init__(self, *, type_path: str, type_oid: ObjectIdentifier,
                 value_path: str, **kwargs
                 ):
...
class DEREqualityValidator(Validator):
    def __init__(self, *, other_node_retriever=Callable[[PDUNode], PDUNode],
                 validation: ValidationFinding, **kwargs
                 ):
    ...
    def validate(self, node):
        ...
        if encode(node.pdu) != encode(other_node.pdu):
            raise ValidationFindingEncountered(
                self.validations[0],
                f'DER encoding of {node.path} and {other_node.path} are '
                f'not equal'
            )


class NodePresenceValidator(Validator):
    def __init__(self, *, node_retriever: Callable[[PDUNode], PDUNode],
                 absence_finding: ValidationFinding = None,
                 presence_finding: ValidationFinding = None, **kwargs
                 ):
