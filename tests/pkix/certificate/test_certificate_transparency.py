...
def test_sctlist_decode():
    ...
    assert all((s.hash_alg == certificate_transparency.HashAlgorithm.SHA256 for s in decoded))

    assert all((s.sig_alg == certificate_transparency.SignatureAlgorithm.ECDSA for s in decoded))
    ...
    for actual, expected in zip(
            ...
            (
                datetime.datetime(2023, 2, 14, 16, 58, 33, 338000, tzinfo=datetime.timezone.utc),
                datetime.datetime(2023, 2, 14, 16, 58, 33, 387000, tzinfo=datetime.timezone.utc),
                datetime.datetime(2023, 2, 14, 16, 58, 33, 402000, tzinfo=datetime.timezone.utc),
            )
    ):
        assert actual == expected
