...
from pkilint.etsi.en_319_412_1 import (
    LegalPersonOrganizationIdentifierValidator,
    NaturalPersonIdentifierValidator,
)
...
class Doc:
    ...
    def get_extension_by_oid(self, _):
        ...
        semantics_info_node = document.PDUNode(
            self, "semanticsInformation", semantic_info, None
        )
        ...
        # noinspection PyTypeChecker
        qc_statements_node = document.PDUNode(
            self, "qCStatements", qc_statements, ext_node
        )
        qc_statements_node.navigate("0.statementInfo").children[
...
def test_orgid_national_scheme():
    values_under_test = ["EI:SE-5567971433"]
...
def _expect_finding_for_natural_person_id_value(value, finding):
    ...
    node = _create_serialnumber_node(
        value, en_319_412_1.id_etsi_qcs_semanticsId_Natural
    )
